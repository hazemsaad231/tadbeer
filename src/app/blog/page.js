...
//    const data = await fetch(`${Api}/services?per_page=100`, { next: { revalidate: 1000 } });
...
import bg from "../../../public/img4.png";
import { Api } from "../components/api/api";
// لا نحتاج لاستيراد revalidatePath هنا، لكن إذا كنت تستخدمها في مكان آخر فلا مشكلة

export default async function Bolgs() {
  
  // القائمة الافتراضية للتدوين (بيانات احتياطية)
  let blog = []; 

  try {
    // محاولة جلب البيانات مع الحفاظ على revalidate: 1000
    const data = await fetch(`${Api}/services?per_page=100`, { 
      next: { revalidate: 1000 } 
    });

    // التحقق من حالة الاستجابة (مثل 200 OK)
    if (data.ok) {
        const Data = await data.json();
        // تحديث متغير البيانات المجلوبة
        blog = Data.data || []; 
    } else {
        // إذا فشل الـ fetch ولكن لم ينهار (مثل 404 أو 500)، يتم تسجيل ذلك ويتم استخدام القائمة الفارغة
        console.warn(`Fetch failed with status: ${data.status}. Using fallback data.`);
    }

  } catch (error) {
    // معالجة أخطاء الشبكة والمهلة (Timeout) هنا لمنع انهيار البناء
    console.error("Critical API fetch error during build (Network Timeout/Failure). Using fallback data.", error.message);
    // إذا حدث خطأ، يظل المتغير 'blog' كقائمة فارغة []
  }
  
  // استخدام البيانات المجلوبة أو البيانات الاحتياطية
  const blogs = blog.filter((it) => it.type === 'blogs')
